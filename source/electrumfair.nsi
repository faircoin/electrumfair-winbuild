...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "c:/electrum/electrumfair-setup.exe"
  ...
  VIAddVersionKey OriginalFilename "electrumfair.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrumfair.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrumfair.exe" "" "$INSTDIR\electrumfair.exe" 0
